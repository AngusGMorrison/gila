...
// KeyReader satisfies editor.KeyReader. It avoids allocations when reading keys from input by
// maintaining a buffer, keyBuf, that is returned to the caller by ReadKey and shared between
// ReadKey calls.
...
// NewKeyReader returns a *KeyReader with an key buffer of len maxKeyBytes. This is the maximum size
// of keypress it must be able to read in bytes.
